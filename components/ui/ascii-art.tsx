...
type AsciiArtProps = {
  ...
  /** The aspect ratio (width/height) of the generated ASCII grid. Defaults to 1.0 (square) */
...
export const AsciiArt: React.FC<AsciiArtProps> = ({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const rows = Math.floor(cols * (charAspectRatio / artAspectRatio));

      canvas.width = cols;
      canvas.height = rows;

      const visualAspect = artAspectRatio;
  ...
  }, [src, resolution, effectiveCharset, objectFit, artAspectRatio]);
