...
const DotField = memo(({
  ...
  lightGradientTo,
  glowColor = '#120F17',
  color, // Extract color so it doesn't go to DOM
  ...rest
}: DotFieldProps) => {
  ...
  propsRef.current = { dotRadius, dotSpacing, cursorRadius, cursorForce, bulgeOnly, bulgeStrength, sparkle, waveAmplitude, gradientFrom, gradientTo, lightGradientFrom, lightGradientTo, resolvedTheme };
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (w > 0 && h > 0) {
        try {
          const isDark = p.resolvedTheme !== 'light';
          const activeFrom = (!isDark && p.lightGradientFrom) ? p.lightGradientFrom : p.gradientFrom;
          const activeTo = (!isDark && p.lightGradientTo) ? p.lightGradientTo : p.gradientTo;

          const grad = ctx.createLinearGradient(0, 0, w, h);
          grad.addColorStop(0, activeFrom || 'rgba(168, 85, 247, 0.35)');
          grad.addColorStop(1, activeTo || 'rgba(180, 151, 207, 0.25)');
          ctx.fillStyle = grad;
        } catch (e) {
          ctx.fillStyle = 'rgba(168, 85, 247, 0.35)'; // Safe fallback
        }
      }
