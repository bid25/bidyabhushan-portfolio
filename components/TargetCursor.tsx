...
const getContainingBlock = (element: Element | null) => {
...
const TargetCursor = ({
  ...
  const forceLeaveRef = useRef<(() => void) | null>(null);
  const pathname = usePathname();

  useEffect(() => {
    if (forceLeaveRef.current) {
  ...
  useEffect(() => {
    ...
    if (hideDefaultCursor) {
      ...
      // Also apply to html to ensure it covers the whole page
      document.documentElement.style.cursor = 'none';
      // Apply to all elements by injecting a style tag
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      forceLeaveRef.current = leaveHandler;
    ...
    return () => {
      ...
      if (cornersRef.current) {
        const corners = Array.from(cornersRef.current);
        gsap.killTweensOf(corners, 'x,y,borderColor');
        const { cornerSize } = constants;
        const positions = [
          { x: -cornerSize * 1.5, y: -cornerSize * 1.5 },
          { x: cornerSize * 0.5, y: -cornerSize * 1.5 },
      ...
      if (dotRef.current) {
        gsap.killTweensOf(dotRef.current, 'backgroundColor,scale');
        gsap.set(dotRef.current, { backgroundColor: cursorColor, scale: 1 });
      }
